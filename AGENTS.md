...
- All four specialist roles cover the entire game, from first launch through expansion, late game, victory, bankruptcy and continued play. A bounded task does not narrow that standing scope. Record missing ending design as a gap rather than inventing completion.
- Use four agents total for substantial whole-game work: root owns Game Designer and integration; the other three own Economy Designer, Level Designer and Narrative Designer respectively. Assign independent bounded tasks with file ownership, questions and completion evidence. Routine small edits do not require delegation. If tools are unavailable, apply the relevant review perspectives locally and state the actual coverage.
- Each agent reads its matching file under `game-agent.md/` and the shared role guide. Root reads all four and owns integration.
