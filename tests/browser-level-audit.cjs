...
for(const stage of ['early','full-map-fixture']){
 ...
 if(end){const last=page.locator('#desk-content button:visible:enabled').last();await last.scrollIntoViewIfNeeded();assert.ok(await last.evaluate(el=>{const a=el.getBoundingClientRect(),r=deskContent.getBoundingClientRect();return a.bottom<=r.bottom+2&&a.top>=r.top-2;}),`${stage}/${language}/${width}/${tab} last action unreachable`);}
