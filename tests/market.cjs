...
ev('state.cash=3000000;window.target=state.enterprise.operations.machines.find(m=>m.loc===7);target.condition=42;target.vault=7654;target.slots[0].stock=3;target.slots[0].value=1500;syncMachine(target);acquireNpc(7);proposeTrade(state.enterprise.trade.quote);completeTrade()');assert.equal(ev('machine(7).stock'),3);assert.equal(ev('machine(7).vault'),7654);assert.equal(ev('machine(7).condition'),42);
