...
function landscape(){const season=sceneSeason(),night=!!state.live&&state.live.elapsed>DAY_MS*.8,newKey=selectedMap+':'+season+':'+night+':'+state.weather+':'+lang;if(key===newKey)return;key=newKey;objects=[];const tree=season===1?'tree':'tree-'+['spring','summer','autumn','winter'][season],land=season===3?'mist':season===2?'sand':'grass';gc.fillStyle=PAL[land];gc.fillRect(0,0,W,H);
 ...
 districtPieces[selectedMap].forEach((kind,i)=>{const [x,y]=lots[i],[sx,sy]=point(x,y);
  if(['garden','riverside','beach','camp','lookout','fair','plaza','market'].includes(kind)){
   ...
   if(kind==='lookout'){object('lookout',sx-5,sy-7);object(tree,sx+42,sy-37);}
...
function addPerson(actors,loc,progress,seed,time,using,worker=false,out=false,cargo=true){const pt=route(loc,progress),x=pt[0]+(worker?18:0),y=pt[1]+(worker?9:0),dir=out?'nw':'se',frame=sceneMotion?Math.floor(time/145+seed)%4:0,name=using?(worker?'worker-use':'customer-use'):worker?`worker-${dir}-${frame}`:`customer-${['blue','leaf','gold'][seed%3]}-${dir}-${frame}`;actors.push({name,x,y,depth:y,kind:worker?'worker':'customer',loc});if(worker&&cargo)actors.push({name:'cart',x:x+13,y:y+5,depth:y+5,kind:'cargo',loc});if(state.weather===2&&sceneSeason()!==3)actors.push({name:'umbrella',x,y:y-19,depth:y+.1,kind:'umbrella',loc});}
function paint(time){landscape();ctx.clearRect(0,0,W,H);ctx.drawImage(ground,0,0);const actors=[],bubbles=[],l=state.live,clock=l?Math.min(DAY_MS,l.elapsed+businessCarry):time;
 ...
 const workerSeen=new Set();for(const [i,j]of jobs.entries()){if(workerSeen.has(j.loc))continue;if(LOCATIONS[j.loc]?.map!==selectedMap||!(j.duration>0))continue;workerSeen.add(j.loc);const progress=Math.max(0,Math.min(1,1-j.remaining/j.duration));addPerson(actors,j.loc,Math.min(1,progress/ .82),i,clock,progress>.82,true,false,j.type!=='collect');}
 const reserved=actors.filter(a=>a.kind==='worker'||a.kind==='cargo').map(a=>[a.x,a.y]);
