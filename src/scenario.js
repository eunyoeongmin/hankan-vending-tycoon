...
launchNew=function(){
 ...
 state=fresh();state.scenario=scenario;state.started=true;state.duration=chosenDuration;state.difficulty=0;state.cash=scenario.rules.playerFunds;state.npc.cash=scenario.rules.rivalFunds;state.npc.owned=[7,14,3,20,10,17].slice(0,scenario.rules.rivalMachines);
...
renderEnterprise=function(force=true){enterpriseBeforeScenario(force);if(enterpriseTab!=='overview')return;let line=$('scenario-run-status');if(!line){line=document.createElement('p');line.id='scenario-run-status';line.className='helper';$('enterprise').querySelector('.enterprise-body')?.append(line);}if(line)line.textContent=state.scenario?T(`설정: ${PRESET_NAMES[state.scenario.preset][0]} · 지수 ${rulesIndex(runRules())} · 경기 수요 ${Math.round(cycleFactor()*100)}% · 비용 물가 ${Math.round(inflationFactor()*100)}%`,`設定：${PRESET_NAMES[state.scenario.preset][1]}・指数 ${rulesIndex(runRules())}・景気需要 ${Math.round(cycleFactor()*100)}%・コスト物価 ${Math.round(inflationFactor()*100)}%`):T('기존 저장: 이전 버전의 시작 조건을 유지합니다.','既存セーブ：旧バージョンの開始条件を保持します。');};

document.addEventListener('click',event=>{if(!event.target.closest('#settings-close'))return;if(state.started&&!state.ended){menuOpen=false;closeModal();if(!state.live&&!livePaused)startBusiness();render();}else{menuOpen=true;openModal('menu');}});

document.addEventListener('click',event=>{if(!event.target.closest('#game-menu-close'))return;menuOpen=false;closeModal();if(state.started&&!state.ended&&!state.live&&!livePaused)startBusiness();render();});
