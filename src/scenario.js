...
const STANDARD_RULES={rivalCompanies:3,playerFunds:350000,rivalFunds:500000,rivalMachines:2,aggression:1,expertise:1,events:2,supply:1,cycle:0,inflation:0};
const LEGACY_RULES={...STANDARD_RULES,rivalCompanies:1,supply:0};
const RULE_PRESETS={easy:{...STANDARD_RULES,rivalCompanies:1,playerFunds:750000,rivalFunds:150000,rivalMachines:1,aggression:0,expertise:0,events:1,supply:0},standard:{...STANDARD_RULES},hard:{...STANDARD_RULES,playerFunds:150000,rivalFunds:1500000,rivalMachines:4,aggression:2,expertise:2,events:3,supply:2,cycle:1,inflation:3},extreme:{...STANDARD_RULES,playerFunds:150000,rivalFunds:3000000,rivalMachines:6,aggression:2,expertise:2,events:3,supply:2,cycle:1,inflation:8}};
...
const RULE_FIELDS=[
 ...
 ['rivalMachines',B('회사당 초기 자판기','1社あたりの初期自販機'),[[1,B('1대','1台')],[2,B('2대','2台')],[4,B('4대','4台')],[6,B('6대','6台')]],B('각 회사의 초기 점유 입지 수. 총 24입지 안에서 배치합니다.','各社の初期占有立地数。計24立地に配置。')],
 ...
 ['expertise',B('경쟁사 운영 능력','競合の運営能力'),[[0,B('낮음','低い')],[1,B('보통','普通')],[2,B('높음','高い')]],B('상대 선택 매력 80/100/125%. 조달 단가는 공통 공급처·계약 조건을 적용합니다.','競合の選択魅力80/100/125%。仕入単価は共通の仕入先・契約条件を適用。')],
...
try{const p=JSON.parse(localStorage.getItem('hankan-start-rules')||'null');if(p&&validScenario(p)){setupRules={...p.rules,rivalCompanies:p.rules.rivalCompanies??1};setupPreset=p.preset;}}catch{}
...
function rulesSummary(r){return RULE_FIELDS.map(f=>`<div class="row"><span>${tr(f[1])}</span><strong>${ruleLabel(f,r[f[0]]??(f[0]==='rivalCompanies'?1:0))}</strong></div>`).join('');}
