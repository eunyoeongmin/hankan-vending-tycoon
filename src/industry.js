...
function companyCapacity(f,m){return 80+(m.level-1)*40+f.research.logistics*20;}
...
marketCustomer=function(map,index){
 ...
 for(const f of [playerFirm(),...rivalFirms().filter(f=>!f.policy.defeated)])for(const m of f.machines){
  ...
  for(const s of m.slots.filter(s=>s.stock>0)){const p=PRODUCTS[s.product],ours=f.id==='player',paid=ours?campaignPrice(s,map):Math.max(800,Math.round(s.price*f.meta.prices[map]/1500));
   ...
   let score=fit*Math.exp((1-paid/p.price)*[2.3,1.5,.9][type])*(.5+m.condition/200)/(1+Math.abs(m.loc-origin)*.6)/4*(1+f.tech.quality*.09+(s.label?f.brand[map]/140:0));
...
function reviveCompany(f){const empty=LOCATIONS.filter(l=>!machine(l.id)&&!rivalLocations().includes(l.id));if(!empty.length)return false;const l=empty.sort((a,b)=>demand({loc:b.id,level:1})/b.cost-demand({loc:a.id,level:1})/a.cost)[0];
...
rivalTurn=function(){if(!industryEnabled())return industryOldTurn();for(const f of rivalFirms()){
 ...
 if(r.until<state.day)f.meta.prices.fill(1500);for(let map=0;map<4;map++)f.brand[map]=Math.max(5,Math.min(100,f.brand[map]+(r.mode==='advertise'&&r.target===map&&r.until>=state.day?4:-.3)));
...
function validIndustry(i,s){
 ...
 if(s.saveVersion!==6||!int(i.activationDay,1,1000000)||!i.researchReports.every(r=>r&&['mono','atlas','nova'].includes(r.id)&&int(r.day,1,1000000)&&['closed','weak','stable','watch'].includes(r.grade)&&['unknown','up','down','flat'].includes(r.trend)))return false;
 ...
 const extra=m=>(m.pendingExpense===undefined||nn(m.pendingExpense))&&(!m.contract||(int(m.contract.supplier,0,2)&&int(m.contract.until,1,1000000)&&Array.isArray(m.contract.prices)&&m.contract.prices.length===6&&m.contract.prices.every(x=>int(x,1,1000000000))));
 if(!extra(i.primary)||!extra(i.player))return false;
 const locations=[...s.machines.map(m=>m.loc),...s.npc.owned],ids=['mono'],machines=[...s.machines,...(s.enterprise.operations?.machines||[])];for(const x of i.companies){if(!meta(x.meta)||!extra(x.meta)||!['atlas','nova'].includes(x.meta.id)||ids.includes(x.meta.id)||!x.npc||!finite(x.npc.cash)||!int(x.npc.acquired,0,1000000000)||!Array.isArray(x.npc.owned)||!x.npc.owned.every(loc=>int(loc,0,23))||!validOperations(x.operations,s)||!validRivalry(x.rivalry)||x.operations.machines.length!==x.npc.owned.length||x.operations.machines.some(m=>!x.npc.owned.includes(m.loc)))return false;ids.push(x.meta.id);locations.push(...x.npc.owned);machines.push(...x.operations.machines);}return new Set(locations).size===locations.length&&machines.every(m=>!m.lease||(int(m.lease.until,1,1000000)&&int(m.lease.rate,0,1000000000)));
}
...
function completeCompanyBuyout(){if(companyBuyoutError())return false;const i=state.enterprise.industry,q=i.buyout,f=rivalById(q.id),e=state.enterprise;
 ...
 for(let p=0;p<PRODUCTS.length;p++){e.warehouse[p].qty+=f.ops.warehouse[p].qty;e.warehouse[p].value+=f.ops.warehouse[p].value;f.ops.warehouse[p]={qty:0,value:0};}for(const o of f.ops.orders)e.orders.push({...o,id:e.nextOrder++,duration:o.duration||o.remaining});f.ops.orders=[];
 for(const m of [...f.machines]){state.machines.push(m);removeRivalMachine(f,m.loc);}for(const key of ['quality','logistics'])e.tech[key]=Math.max(e.tech[key],f.tech[key]);for(const key of ['products','logistics','efficiency'])state.research[key]=Math.max(state.research[key],f.research[key]);for(let map=0;map<4;map++)e.brand[map]=Math.max(e.brand[map],f.brand[map]);e.capital+=f.meta.capital;
