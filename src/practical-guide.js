...
const GUIDE_LESSONS=[
 ...
 ['manage',B('첫 판매 확인','最初の販売'),B('이제 문을 열어 볼까요? 위의 재개 버튼을 누르면 영업 시간이 흘러요. 지도와 아래 영업 상황을 보며 첫 손님을 기다려 봐요. 판매가 생기면 제가 멈출게요.','それでは営業を始めましょうか。上の再開ボタンで時間が進みます。地図と下の営業状況を見ながら、最初のお客さまを待ちましょう。売れたら私が止めますね。')],
...
function guideSpotlight(){
 const visible=guideActive()&&!modalView&&!menuOpen;guideSpot.style.display=visible?'block':'none';if(!visible)return;
...
function guidePaint(){const active=guideActive(),g=guideState();guidePanel.hidden=!active;document.body.classList.toggle('guide-active',active);
 ...
 guideLockControls(active);guideSpotlight();if(!active)return;
