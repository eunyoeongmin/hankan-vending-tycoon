...
worldEvents=function(){const e=state.enterprise;state.weather=e.forecast??0;e.forecast=Math.floor(enterpriseRandom()*4);if(sceneSeason()===3&&e.forecast===1)e.forecast=3;e.index=Math.max(.75,Math.min(1.4,e.index+(enterpriseRandom()-.5)*.08));for(let map=0;map<MAPS.length;map++){e.brand[map]=Math.max(5,e.brand[map]-.3);if(e.ads[map]>0){e.ads[map]--;e.brand[map]=Math.min(100,e.brand[map]+4);}const pressure=state.machines.filter(m=>LOCATIONS[m.loc].map===map).length;/* Rival prices are set by its funded strategy. */}oldWorldEventsEnterprise();if(e.autoOrder)for(let p=0;p<PRODUCTS.length;p++){if(!state.machines.some(m=>m.slots?.some(s=>s.product===p)))continue;if(e.warehouse[p].qty<40&&!e.orders.some(o=>o.product===p))orderGoods(p,Math.max(100,SUPPLIERS[e.supplier].min));}};
finishBusiness=function(){
 ...
 if(industryEconomy()){settleCompanyDebt(playerFirm());}
 else if(state.cash>=interest+state.bank.arrears){state.cash-=interest+state.bank.arrears;state.bank.arrears=0;state.bank.missed=0;}else{state.bank.arrears+=interest;if(interest||state.bank.arrears)state.bank.missed++;}
...
function enterpriseAction(action,p){const e=state.enterprise;if(!state.started||state.ended)return;const m=machine(selected);
 ...
 if(action==='ad'){const map=Number(p);if(int(map,0,MAPS.length-1)&&e.ads[map]===0&&state.cash>=25000){enterpriseExpense(25000,'advertising');e.ads[map]=5;e.brand[map]=Math.min(100,e.brand[map]+5);}}
...
renderManage=function(){const m=machine(selected);if(!m){oldManageEnterprise();return;}ensureSlots(m);$('manage-title').textContent=tr(LOCATIONS[selected].name);$('manage-tag').textContent=T('4슬롯 자판기','4スロット自販機');const e=state.enterprise,locked=!state.started||state.ended;
 const html=`<p class="helper">${T('슬롯별 상품·가격을 설정하세요. 상품 교체 시 재고는 창고로 돌아갑니다.','スロットごとに商品と価格を設定。商品変更時の在庫は倉庫に戻ります。')}</p>${m.slots.map((s,i)=>`<div class="slot-card" data-slot-card="${i}"><label for="slot-${i}">${T('진열','陳列')} ${i+1} · ${s.stock}/${Math.floor(capacity(m)/4)}</label><select class="full" id="slot-${i}" data-slot="${i}" ${disabled(locked)}>${PRODUCTS.map((p,id)=>`<option value="${id}" ${s.product===id?'selected':''} ${disabled(!!refRetailMachineReason(m,id))}>${tr(p.name)}${refRetailMachineReason(m,id)?' · '+refRetailMachineReason(m,id):''}</option>`).join('')}</select><div class="price-controls"><button data-enterprise="price" data-param="${i}:-100" ${disabled(locked||s.price<=800)}>−</button><strong>${money(s.price)}</strong><button data-enterprise="price" data-param="${i}:100" ${disabled(locked||s.price>=4000)}>＋</button></div><small>${T('현재 결제 가격','現在の支払価格')} ${money(campaignPrice(s,LOCATIONS[selected].map))}<br>${T('재고 평균 원가','在庫平均原価')} ${money(s.stock?s.value/s.stock:PRODUCTS[s.product].cost)} · ${T('창고','倉庫')} ${e.warehouse[s.product].qty}</small><button class="small full" data-enterprise="label" data-param="${i}" ${disabled(locked||e.tech.quality<2)}>${s.label?T('자체 브랜드 적용 중','自社ブランド適用中'):T('자체 브랜드 · 품질 Lv.2','自社ブランド・品質Lv.2')}</button></div>`).join('')}${row(T('기기 상태','機械の状態'),Math.round(m.condition)+'%')}<button class="small full" data-enterprise="repair" ${disabled(locked||m.condition>=100||(chainEnabled()?chainFirm(playerFirm()).orders.some(o=>o.kind==='repair'&&o.loc===m.loc):state.cash<15000))}>${chainEnabled()?T('정비 예약','整備予約'):T('예방 정비 · $15','予防整備・$15')}</button><p class="helper">${T('상태 25% 미만이면 구매 실패가 늘어납니다.','状態25%未満では購入失敗が増えます。')}</p>${row(T('보관 매출','保管売上'),money(m.vault))}<button class="primary full" data-job="collect" data-loc="${m.loc}" ${disabled(locked||m.vault<=0||state.jobs.some(j=>!j.staff))}>${T('직접 회수하러 이동','自分で回収に向かう')}</button><button class="full" data-action="refill" id="refill-button" ${disabled(locked||!hasRestock(m)||state.jobs.some(j=>!j.staff))}>${T('창고에서 보충하러 이동','倉庫から補充に向かう')}</button><p class="helper">${e.emergency?T('창고 부족분은 25% 비싼 긴급 매입으로 보충합니다.','倉庫の不足分は25%割高の緊急仕入れで補充します。'):T('창고 재고만 사용합니다. 발주 후 입고를 기다리세요.','倉庫在庫のみ使用。発注後、入庫を待ちましょう。')}</p><button class="full" data-action="upgrade" ${disabled(locked||!chainEnabled()&&(m.level>=3||state.cash<upgradeCost(m)))}>${chainEnabled()?T('기기 조달·설치','機械調達・設置'):T('용량 확장','容量拡張')+' Lv.'+m.level+'/3 · '+money(upgradeCost(m))}</button><button class="small full" data-action="sell" ${disabled(locked||state.machines.length<=1)}>${T('기기 매각','機械売却')}</button>`;patchPanel($('manage'),html+industryLeaseHTML(m));
};
...
function validEnterprise(e,s){
 ...
 return e.version===1&&(!e.reference||validReference(e.reference,s))&&(!e.chain||validChain(e.chain,s))&&(!e.expansion||validExpansion(e.expansion,s))&&(!e.industry||validIndustry(e.industry,s))&&(!e.trade||validTrade(e.trade))&&(!e.operations||validOperations(e.operations,s))&&(!e.rivalry||validRivalry(e.rivalry))&&int(e.rng,0,4294967295)&&Array.isArray(e.warehouse)&&e.warehouse.length===PRODUCTS.length&&e.warehouse.every(w=>w&&int(w.qty,0,100000)&&nonnegative(w.value))&&Array.isArray(e.orders)&&e.orders.length<=1000&&e.orders.every(o=>int(o.id,1,1e9)&&int(o.product,0,PRODUCTS.length-1)&&int(o.qty,1,1000)&&nonnegative(o.value)&&finite(o.remaining)&&o.remaining>0&&finite(o.duration)&&o.duration>0)&&int(e.nextOrder,1,1e9)&&e.tech&&['quality','logistics'].every(k=>int(e.tech[k],0,3))&&['brand','ads','rivalPrices'].every(k=>Array.isArray(e[k])&&e[k].length===MAPS.length&&e[k].every(nonnegative))&&finite(e.index)&&e.index>=.75&&e.index<=1.4&&typeof e.emergency==='boolean'&&typeof e.autoOrder==='boolean'&&int(e.supplier,0,2)&&finite(e.expenseToday)&&int(e.expenseDay,1,1000000)&&nonnegative(e.capital)&&int(e.lastClosed,0,1000000)&&Array.isArray(e.reports)&&e.reports.length<=60&&e.reports.every(r=>int(r.day,1,1000000)&&finite(r.net)&&finite(r.cash)&&nonnegative(r.revenue)&&Array.isArray(r.rows))&&Array.isArray(e.ledger)&&e.ledger.length<=60&&e.ledger.every(x=>int(x.day,1,1000000)&&['purchase','delivery','emergency','collection','investment','advertising','maintenance'].includes(x.kind)&&finite(x.amount))&&s.machines.every(m=>!m.slots||(Array.isArray(m.slots)&&m.slots.length===4&&m.slots.every(x=>int(x.product,0,PRODUCTS.length-1)&&int(x.stock,0,10000)&&int(x.price,800,4000)&&nonnegative(x.value)&&typeof x.label==='boolean')&&m.slots.reduce((n,x)=>n+x.stock,0)===m.stock&&finite(m.condition)&&m.condition>=0&&m.condition<=100));
}
