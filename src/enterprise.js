...
function customerPurchase(m,r,i){
 ...
 const scores=available.map(s=>{const p=PRODUCTS[s.product],ratio=s.price/p.price;let fit=(p.fit[kind]||1)*(type===0&&s.product===0?1.25:type===1&&s.product===1?1.35:type===2&&s.product===3?1.3:1);
 ...
 const competitor=rivals*.17*(1500/e.rivalPrices[map])*state.pressure.filter(p=>p.map===map).reduce((n,p)=>n/p.factor,1)+(localOwn-1)*.04;
 ...
 const unit=chosen.value/chosen.stock;chosen.stock--;chosen.value=Math.max(0,chosen.value-unit);m.total++;m.vault+=chosen.price;m.condition=Math.max(0,m.condition-.045);state.totalSold++;r.served++;r.revenue+=chosen.price;r.cost+=unit;r.bought.push({i,price:chosen.price});const stat=r.products[chosen.product];stat.sold++;stat.revenue+=chosen.price;stat.cost+=unit;syncMachine(m);
}
...
finishBusiness=function(){
 ...
 e.lastClosed=day;e.reports.unshift({day,revenue:t.revenue,net,cash:state.cash,extra:extraExpense,overhead,rows:snapshot});e.reports=e.reports.slice(0,60);
