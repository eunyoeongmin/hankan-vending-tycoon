...
function validOperations(o,s){
 ...
 if(!o||o.version!==1||!arr(o.machines,LOCATIONS.length)||new Set(o.machines.map(m=>m.loc)).size!==o.machines.length||!o.machines.every(m=>m&&int(m.loc,0,LOCATIONS.length-1)&&int(m.level,1,3)&&int(m.total,0,1e12)&&nn(m.vault)&&int(m.stock,0,10000)&&int(m.product,0,5)&&int(m.price,800,4000)&&nn(m.condition)&&m.condition<=100&&arr(m.slots,4)&&m.slots.length===4&&m.slots.every(x=>x&&int(x.product,0,5)&&int(x.stock,0,10000)&&int(x.price,800,4000)&&nn(x.value)&&typeof x.label==='boolean')&&m.slots.reduce((n,x)=>n+x.stock,0)===m.stock))return false;
 if(!arr(o.warehouse,6)||o.warehouse.length!==6||!o.warehouse.every(w=>w&&int(w.qty,0,100000)&&nn(w.value))||!arr(o.orders,1000)||!o.orders.every(x=>x&&int(x.product,0,5)&&int(x.qty,1,1000)&&nn(x.value)&&nn(x.remaining)&&x.remaining>0)||!arr(o.jobs,48)||!o.jobs.every(x=>x&&['collect','restock'].includes(x.type)&&int(x.loc,0,LOCATIONS.length-1)&&nn(x.remaining)&&x.remaining>0)||!finite(o.lastProfit)||!int(o.settledDay,0,1000000))return false;
 const m=o.market;if(!m)return true;
 return int(m.day,1,s.day)&&typeof m.legacy==='boolean'&&nn(m.payroll)&&finite(m.extra)&&arr(m.rows,LOCATIONS.length)&&m.rows.every(r=>validLive({day:s.day,elapsed:0,rows:[r]},{day:s.day,ended:false}))&&arr(m.districts,MAPS.length)&&(m.legacy?m.districts.length===0:m.districts.length===MAPS.length&&m.districts.every(d=>d&&int(d.n,0,5000)&&int(d.attempted,0,d.n)&&int(d.player,0,d.attempted)&&int(d.rival,0,d.attempted)&&int(d.none,0,d.attempted)&&d.player+d.rival+d.none===d.attempted));
}
