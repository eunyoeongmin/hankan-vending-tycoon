...
function beRecall(q,h){const lot=q.lot||q.batch;for(const f of allFirms()){let loss=0,qty=0;for(const b of [...beBuckets(f),...beTransitBuckets(f)]){const take=refTake(b,1e9,x=>(x.lot||x.id)===lot);qty+=take.reduce((n,x)=>n+x.qty,0);loss+=take.reduce((n,x)=>n+x.value,0);refSyncBucket(b,'stock'in b);}refFirm(f).work=refFirm(f).work.filter(j=>j.kind!=='purchase'||!j.batches||j.qty>0);for(const m of f.machines)syncMachine(m);if(qty){expenseBookLoss(f,loss);h.waste+=qty;h.cost+=loss;if(f.id!=='player'){const payment=Math.min(Math.max(0,state.cash),Math.ceil(loss));if(payment>0){state.cash-=payment;refIncome(f,payment,'recall-compensation');refPost(playerFirm(),'recall-compensation',-payment);expenseBookLoss(playerFirm(),payment);h.spent+=payment;}if(payment<loss){expenseBookLoss(playerFirm(),loss-payment);expenseBookLoss(f,-(loss-payment));refFirm(playerFirm()).bills.push({id:refId(),to:f.id,amount:loss-payment,due:state.day+7,kind:'recall'});refFirm(f).receivables.push({id:refId(),invoice:refFirm(playerFirm()).bills.at(-1).id,from:'player',amount:loss-payment,due:state.day+7});}}beMessage(h,`${companyNameFor(f.id,0)}: 대상 배치 ${qty}개 회수·폐기`,`${companyNameFor(f.id,1)}：対象ロット${qty}本回収・廃棄`);}}
}
...
function beCompleteTask(t){const f=playerFirm(),r=refFirm(f),m=machine(t.loc),w=chainFirm(f).staff.find(w=>w.id===t.worker),h=beRecord(t.serial);if(!h){t.due=state.day+1;return;}
 ...
 if(t.kind==='repair'&&m){m.condition=t.amount===100?100:Math.min(100,m.condition+t.amount);if(t.clear)state.complaints=state.complaints.filter(c=>c.loc!==t.loc||c.kind==='request');beTrust(q,2);beMessage(h,'정비 완료 · 영업 재개','整備完了・営業再開');}
