...
const mgBeforeRoom=companyBorrowRoom;companyBorrowRoom=function(f){if(!mgEnabled())return mgBeforeRoom(f);const r=refFirm(f);if(state.ended||f.meta.credit<40||r.notes.length>=12||r.notes.some(n=>n.missed)||r.bills.some(b=>b.due<state.day))return 0;const room=Math.floor(Math.max(0,Math.min(refWorld().bankCash,mgCreditLimit(f)-mgDebt(f))));return room>=10000?room:0;};
// Recovery covers obligations payable now, not the entire future loan balance.
function mgPayableNow(f){const r=refFirm(f);return r.bills.filter(b=>b.due<=state.day).reduce((n,b)=>n+b.amount,0)+r.notes.reduce((sum,n)=>sum+(n.due<=state.day?n.principal:Math.min(n.principal,n.duePrincipal||0)),0);}
const mgBeforeRecoverable=companyRecoverable;companyRecoverable=function(f){const value=mgBeforeRecoverable(f);if(!mgEnabled())return value;return value+f.bank.arrears+(state.day>=f.meta.due?f.bank.principal:0)-mgPayableNow(f);};
