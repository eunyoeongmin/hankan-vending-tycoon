...
let deskTab='manage';
function visibleDeskTabs(){return DESK_TABS.filter(t=>!t[2]||t[2]());}
function selectDesk(tab){if(!visibleDeskTabs().some(x=>x[0]===tab))return;deskTab=tab;
...
function updateDesk(){
 ...
 patchPanel(deskNav,visibleDeskTabs().map(([id,name])=>`<button data-desk="${id}" aria-pressed="${deskTab===id}" class="${deskTab===id?'primary':''}">${tr(name)}</button>`).join(''));
